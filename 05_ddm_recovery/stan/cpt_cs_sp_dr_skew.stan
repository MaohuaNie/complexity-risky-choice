...
parameters {
  vector[8] mu;                                     // population-level means for beta, theta, threshold, ndt, gamma, sp, zeta, delta_gamma
...
transformed parameters {
  matrix[8, L] participant_params;                  // participant-specific parameters (beta, theta, threshold, ndt, gamma, sp, zeta, delta_gamma)
  ...
  vector[L] delta_gamma_t;                          // transformed delta_gamma for each participant
  ...
  for (n in 1:N) {
    ...
    real delta_gamma = delta_gamma_t[participant[n]];
    real gamma_complex = log1p_exp(gamma + delta_gamma);
    real gamma_simple = log1p_exp(gamma - delta_gamma);


    // Compute utilities for options A and B
    u_complex[n] = pweight(p_complex[n,1], gamma_complex) * pow(o_complex[n,1], beta) + (1 - pweight(p_complex[n,1], gamma_complex)) * pow(o_complex[n,2], beta);
    u_simple[n] = pweight(p_simple[n,1], gamma_simple) * pow(o_simple[n,1], beta) + (1 - pweight(p_simple[n,1], gamma_simple)) * pow(o_simple[n,2], beta);
