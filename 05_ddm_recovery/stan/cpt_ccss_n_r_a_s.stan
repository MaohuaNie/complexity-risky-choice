...
parameters {
  vector[9] mu;                                     // population-level means for beta, theta, threshold, ndt, gamma, delta_beta, delta_theta, delta_threshold, delta_gamma
  vector<lower=0>[9] sigma;                         // population-level standard deviations
  cholesky_factor_corr[9] L_corr;                   // Cholesky factor of correlation matrix
  matrix[9, L] z;                                   // unscaled participant-level deviations
}

transformed parameters {
  matrix[9, L] participant_params;                  // participant-specific parameters (beta, theta, threshold, ndt, gamma, delta_beta, delta_theta, delta_threshold, delta_gamma)
  vector[L] beta_base_t;                            // base beta for each participant
  vector[L] theta_base_t;                           // base theta for each participant
  vector[L] threshold_base_t;                       // base threshold for each participant
  vector[L] ndt_t_p;                                // transformed ndt for each participant
  vector[L] gamma_base_t;                           // base gamma for each participant
  vector[L] delta_beta_t;                           // delta_beta for each participant
  vector[L] delta_theta_t;                          // delta_theta for each participant
  vector[L] delta_threshold_t;                      // delta_threshold for each participant
  vector[L] delta_gamma_t;                          // delta_gamma for each participant
  ...
  for (l in 1:L) {
    beta_base_t[l] = participant_params[1, l];  // Keep untransformed
    theta_base_t[l] = participant_params[2, l];  // Keep untransformed
    threshold_base_t[l] = participant_params[3, l];  // Keep untransformed
    ndt_t_p[l] = log1p_exp(participant_params[4, l]);
    gamma_base_t[l] = participant_params[5, l];  // Keep untransformed
    delta_beta_t[l] = participant_params[6, l];
    delta_theta_t[l] = participant_params[7, l];
    delta_threshold_t[l] = participant_params[8, l];
    delta_gamma_t[l] = participant_params[9, l];
  }
  ...
  for (n in 1:N) {
    ...
    real theta = log1p_exp(theta_base_t[participant[n]] + delta_theta_t[participant[n]] * con[n]);
    ...
    real gamma = log1p_exp(gamma_base_t[participant[n]] + delta_gamma_t[participant[n]] * con[n]);
...
model {
  ...
  mu[6] ~ normal(0, 3);  // Prior for delta_beta's population mean
  mu[7] ~ normal(0, 3);  // Prior for delta_theta's population mean
  mu[8] ~ normal(0, 3);  // Prior for delta_threshold's population mean
  mu[9] ~ normal(0, 3);  // Prior for delta_gamma's population mean
...
generated quantities {
  matrix[9, 9] Omega;                               // correlation matrix
