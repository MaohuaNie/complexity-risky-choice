...
parameters {
  vector[6] mu;                             // population-level means for beta, theta, threshold, ndt, eta, rel_sp
...
transformed parameters {
  matrix[6, L] participant_params;          // participant-specific parameters
...
model {
  ...
  mu[6] ~ normal(0, 1);  // Prior for eta's population mean
  ...
  // Priors for participant-level deviations
  to_vector(z) ~ normal(0, 1);  // standard normal prior for non-centered parameterization
  // Likelihood
