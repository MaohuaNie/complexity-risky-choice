...
parameters {
  vector[8] mu;                                     // population-level means for beta, theta, threshold, ndt, gamma, delta_theta, delta_threshold, delta_gamma
  vector<lower=0>[8] sigma;                         // population-level standard deviations
  cholesky_factor_corr[8] L_corr;                   // Cholesky factor of correlation matrix
  matrix[8, L] z;                                   // unscaled participant-level deviations
}

transformed parameters {
  matrix[8, L] participant_params;                  // participant-specific parameters (beta, theta, threshold, ndt, gamma, delta_theta, delta_threshold, delta_gamma)
  ...
  for (l in 1:L) {
    ...
    delta_theta_t[l] = participant_params[6, l];
    delta_threshold_t[l] = participant_params[7, l];
    delta_gamma_t[l] = participant_params[8, l];
  }
...
model {
  ...
  mu[6] ~ normal(0, 3);  // Prior for delta_theta's population mean
  mu[7] ~ normal(0, 3);  // Prior for delta_threshold's population mean
  mu[8] ~ normal(0, 3);  // Prior for delta_gamma's population mean
...
generated quantities {
  matrix[8, 8] Omega;                               // correlation matrix
