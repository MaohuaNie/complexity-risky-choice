...
data {
  ...
  array[N, 2] real o_risky;                                    // outcomes for option complex 
  array[N, 2] real o_safe;                                    // outcomes for option simple 
  array[N, 2] real p_risky;                                    // probabilities for option complex
  array[N, 2] real p_safe;                                    // probabilities for option simple
  real<lower=0, upper=1> starting_point;            // starting point for diffusion model, fixed value
}

parameters {
  vector[5] mu;                                     // population-level means for beta, theta, threshold, ndt, gamma
  vector<lower=0>[5] sigma;                         // population-level standard deviations
  cholesky_factor_corr[5] L_corr;                   // Cholesky factor of correlation matrix
  matrix[5, L] z;                                   // unscaled participant-level deviations
}

transformed parameters {
  matrix[5, L] participant_params;                  // participant-specific parameters (beta, theta, threshold, ndt, gamma)
...
generated quantities {
  matrix[5, 5] Omega;                               // correlation matrix
