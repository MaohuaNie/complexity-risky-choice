...
parameters {
  vector[6] mu;                             // population-level means for beta, theta, threshold, ndt, eta, delta_eta
...
transformed parameters {
  matrix[6, L] participant_params;          // participant-specific parameters (beta, theta, threshold, ndt, eta, delta_eta)
  ...
  for (n in 1:N) {
    ...
    real delta_eta = participant_params[6, participant[n]];

    drift_t[n] = theta * (evd[n] + beta * sdd[n] + ((eta + delta_eta) * skew_c[n] - (eta - delta_eta) * skew_s[n]));
...
model {
  ...
  mu[6] ~ normal(0, 3);  // Prior for eta's population mean
