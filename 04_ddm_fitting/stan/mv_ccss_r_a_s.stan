...
parameters {
  vector[8] mu; // (beta, theta, threshold, ndt, eta, delta_beta, delta_threshold, delta_eta)
...
transformed parameters {
  ...
  for(n in 1:N) {
    ...
    real theta = log(1 + exp(participant_params[2, participant[n]]));
    real threshold = log(1 + exp(participant_params[3, participant[n]] + participant_params[7, participant[n]]*con[n]));
    real ndt = log(1 + exp(participant_params[4, participant[n]]));
    real eta = participant_params[5, participant[n]] + participant_params[8, participant[n]]*con[n];
...
model {
  ...
  mu[4] ~ normal(-1, 1);
  mu[5] ~ normal(0, 3);
  mu[6] ~ normal(0, 3);  // delta_beta
  mu[7] ~ normal(0, 3);  // delta_threshold
  mu[8] ~ normal(0, 3);  // delta_eta
...
generated quantities {
  matrix[8,8] Omega = multiply_lower_tri_self_transpose(L_corr);
  vector[N] log_lik;
  for(n in 1:N) {
