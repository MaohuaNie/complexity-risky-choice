...
parameters {
  vector[7] mu;                                     // population-level means for beta, theta, threshold, ndt, gamma, zeta, delta_gamma
...
transformed parameters {
  matrix[7, L] participant_params;                  // participant-specific parameters (beta, theta, threshold, ndt, gamma, zeta, delta_gamma)
  ...
  for (n in 1:N) {
    ...
    // Compute trial-by-trial drift rates (FIXED FORMULA)
...
model {
  ...
  mu[6] ~ normal(0, 3);  // Prior for zeta's population mean
