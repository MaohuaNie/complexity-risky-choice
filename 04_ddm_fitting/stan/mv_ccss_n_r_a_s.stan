...
parameters {
  vector[9] mu;                             // population-level means for beta, theta, threshold, ndt, eta, delta_beta, delta_theta, delta_threshold, delta_eta, 
...
transformed parameters {
  matrix[9, L] participant_params;          // participant-specific parameters (beta, theta, threshold, ndt, eta)
  ...
  for (n in 1:N) {
    ...
    real threshold = log(1 + exp(participant_params[3, participant[n]] + participant_params[8, participant[n]]*con[n]));
    real ndt = log(1 + exp(participant_params[4, participant[n]]));
    real eta = participant_params[5, participant[n]] + participant_params[9, participant[n]]*con[n];
