...
parameters {
  vector[5] mu;                             // population-level means for beta, theta, threshold, ndt, eta
...
transformed parameters {
  matrix[5, L] participant_params;          // participant-specific parameters (beta, theta, threshold, ndt, eta)
  ...
  for (n in 1:N) {
    ...
    drift_t[n] = theta * (evd[n] + beta * sdd[n] + eta * skew[n]);  // weighting evd and sdd by theta
